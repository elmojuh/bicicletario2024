...
describe('Tranca em Controller', () => {
    ...
    //     const dto = novaTrancaDTO();
    //     trancaServiceMock.prototype.integrarNaRede = jest.fn().mockResolvedValue(dto);
    //
    //     const res = await request(app)
    //         .post('/api/tranca/integrar')
    //         .send(dto);
    //     expect(res.statusCode).toBe(200);
    // });
    ...
    //         .send(dto);
    //     expect(res.statusCode).toBe(422);
    //     expect(res.body).toEqual({ codigo: '422', mensagem: 'Erro ao integrar tranca' });
    // });
    //
    ...
    // });
    //
    // it('deve alterar status da tranca', async () => {
    //     const dto = novaTrancaDTO();
    //     const tranca = novaTranca(1);
    //     tranca.atualizar(dto);
    //
